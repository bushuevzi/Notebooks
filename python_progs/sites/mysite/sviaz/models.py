...
from conda_build.build import verbose
...
class Kpd(models.Model):
    kod = models.AutoField(db_column='Код', primary_key=True)  # Field name made lowercase.
    #platform = models.ForeignKey('Platform', models.DO_NOTHING, db_column='№АЗС', blank=True, null=True)  # Field name made lowercase. Field renamed to remove unsuitable characters. Field renamed because it started with '_'.
    #platform = models.ForeignKey(Platform, db_column='№АЗС', on_delete=models.CASCADE, db_constraint=False)   # Field name made lowercase. Field renamed to remove unsuitable characters. Field renamed because it started with '_'.
    platform = models.CharField(db_column='№АЗС', max_length=255, blank=True, null=True)
    ...
    provider = models.CharField(db_column='провайдер',max_length=255, blank=True, null=True)
    ...
    note = models.CharField(db_column='Примечание', max_length=511, blank=True, null=True)  # Field name made lowercase.
    number_in_docs_from_prov = models.IntegerField(db_column='Номер в документах оператора', blank=True, null=True)  # Field name made lowercase. Field renamed to remove unsuitable characters.
    ...
    #def __str__(self):
    #    return '%s %s %s %s %s %s %s' (self.platform, self.service, self.throughput, self.tarif, self.last_mile, self.provider, self.contract_number_legal, self.b_z)
    class Meta:
        managed = False
        db_table = 'Common_KPD'
...
class Phones(models.Model):
    kod = models.AutoField(db_column='Код', primary_key=True)  # Field name made lowercase.
    platform = models.ForeignKey('Platform', models.DO_NOTHING, db_column='Объект', blank=True, null=True)  # Field name made lowercase.
